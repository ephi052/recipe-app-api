...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_update_recipe_assign_tag(self):
        ...
        tag_lanch = Tag.objects.create(user=self.user, name='Lanch')
        payload = {
            "tags": [{'name': 'Lanch'}],
        }
        url = detail_url(recipe.id)
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertIn(tag_lanch, recipe.tags.all())
    ...
    def test_filter_by_tags(self):
        ...
        filter_params = {'tags': f'{tag1.id},{tag2.id}'}
        res = self.client.get(RECIPES_URL, filter_params)
    ...
    def test_filter_by_ingredients(self):
        ...
        recipe2 = create_recipe(user=self.user, title='Aubergine with tahini')
        ingredient1 = Ingredient.objects.create(user=self.user, name='Lemon')
        ingredient2 = Ingredient.objects.create(user=self.user, name='Fish Sauce')
        recipe1.ingredients.add(ingredient1)
        recipe2.ingredients.add(ingredient2)
        recipe3 = create_recipe(user=self.user, title='Fish and chips')

        filter_params = {'ingredients': f'{ingredient1.id},{ingredient2.id}'}
        res = self.client.get(RECIPES_URL, filter_params)
